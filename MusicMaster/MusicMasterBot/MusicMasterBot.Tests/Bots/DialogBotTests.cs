...
namespace MusicMasterBot.Tests.Bots
{
    public class DialogBotTests
    {
        ...
        public async Task LogsInformationToILogger()
        {
            ...
            var mockLogger = new Mock<ILogger<MusicBot<Dialog>>>();
            mockLogger.Setup(x =>
                x.Log(It.IsAny<LogLevel>(), It.IsAny<EventId>(), It.IsAny<object>(), null, It.IsAny<Func<object, Exception, string>>()));

            // Run the bot
            var sut = new MusicBot<Dialog>(conversationState, userState, mockRootDialog.Object, mockLogger.Object);
        ...
        public async Task SavesTurnStateUsingMockWithVirtualSaveChangesAsync()
        {
            ...
            // Act
            var sut = new MusicBot<Dialog>(mockConversationState.Object, mockUserState.Object, mockRootDialog.Object, mockLogger.Object);
            var testAdapter = new TestAdapter();
            var testFlow = new TestFlow(testAdapter, sut);
            await testFlow.Send("Hi").StartTestAsync();
